...
class GRNet(torch.nn.Module):
    def __init__(self):
        ...
        self.fc14 = torch.nn.Linear(112, 12)
    ...
    def forward(self, data):
        ...
        # print(sparse_cloud.size())      # torch.Size([batch_size, 512, 3])
        point_features_32 = self.feature_sampling(sparse_cloud, pt_features_32_r).view(-1, 512, 256)
        # print(point_features_32.size()) # torch.Size([batch_size, 512, 256])
        point_features_16 = self.feature_sampling(sparse_cloud, pt_features_16_r).view(-1, 512, 512)
        # print(point_features_16.size()) # torch.Size([batch_size, 512, 512])
        point_features_8 = self.feature_sampling(sparse_cloud, pt_features_8_r).view(-1, 512, 1024)
        # print(point_features_8.size())  # torch.Size([batch_size, 512, 1024])
        point_features = torch.cat([point_features_32, point_features_16, point_features_8], dim=2)
        # print(point_features.size())    # torch.Size([batch_size, 512, 1792])
        point_features = self.fc11(point_features)
        # print(point_features.size())    # torch.Size([batch_size, 512, 1792])
        point_features = self.fc12(point_features)
        # print(point_features.size())    # torch.Size([batch_size, 512, 448])
        point_features = self.fc13(point_features)
        # print(point_features.size())    # torch.Size([batch_size, 512, 112])
        point_offset = self.fc14(point_features).view(-1, 2048, 3)
        # print(point_features.size())    # torch.Size([batch_size, 2048, 3])
        dense_cloud = sparse_cloud.unsqueeze(dim=2).repeat(1, 1, 4, 1).view(-1, 2048, 3) + point_offset
